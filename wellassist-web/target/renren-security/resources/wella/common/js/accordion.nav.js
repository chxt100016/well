    ...
    $('.accordion-nav li').each(function () {
        if ($(this).children('ul').length > 0) {
            if ($(this).children('ul').is(":visible")) {
                $(this).prepend('<img src="resources/chonsurim/images/imgOnOpen.png" />');
            }
            else {
                $(this).prepend('<img src="resources/chonsurim/images/imgOffClosed.png" />');
            }
        }
    });
    ...
    */
    $('.accordion-nav img').click(function() {
        if ($(this).parent('li').children('ul').html() != null) {
            ...
            $(this).parent('li').parent('ul').children('li').children('img').attr('src', 'resources/chonsurim/images/imgOffClosed.png');
            $(this).delay(100).is(':hidden');
            if ($(this).parent('li').children('ul').css('display') == "block") {
                $(this).parent('li').children('ul').hide(slideSpeed);
                $(this).attr('src', 'resources/chonsurim/images/imgOffClosed.png');
            } else {
                $(this).parent('li').children('ul').show(slideSpeed);
                $(this).attr('src', 'resources/chonsurim/images/imgOnOpen.png');
            }
            return false;
        }
