				...
				<div class="item-fore" style="width: 100%;margin-left:0px;">
					<table style="width: 98%;">
						<tr>
							...
							<td style="">
							<label class="radio fl"><input type="radio" name="dlg_state" value="2" />&nbsp;通过</label>
							<label class="radio fl ml10"><input type="radio" name="dlg_state" value="4" />&nbsp;不通过</label>
							</td>
						</tr>
						<tr>
							<td style="width:70px;">供货方 : </td>
							<td style="">
							<select id="dlg_gonghuofang" name="dlg_gonghuofang" data-placeholder="请输入供货方"
							 class="chzn-select"  style="">
							<option value=""></option>
							<c:forEach var="item" items="${arrayList}" varStatus="status">
								<option value="${item.id}">${item.regName}</option>
							</c:forEach>
						</select>
							</td>
						</tr>
...
<script type=text/javascript>
	...
	function openStatusDialog(title,state,content,processFunc){
	    ...
	    $("input[name='dlg_state']").eq(0).attr('checked',true);
	    ...
	    $("#dlgBtnConfirm").unbind("click").bind("click", function(){
			if (processFunc && typeof(processFunc) == "function"){
				...
				var gonghuofang = $("#dlg_gonghuofang").val();
				if(state == 4 && content == ''){
					 alert("请输入不通过原因");
					 return;
				}
				if(state == 2 && gonghuofang == ''){
					 alert("请输入审核人");
					 return;
				}
				
				processFunc(state,content,gonghuofang);
				$('#dlg-edit').hide();
			} 
		});
	}
	$("input[name='dlg_state']").click(function() {
		if($(this).val()==1){
			...
			$("#dlg_gonghuofang").removeAttr("disabled");
			
		}else{
			$("#dlg-edit_content").removeAttr("disabled");
			$("#dlg_gonghuofang").attr("disabled", "disabled");
