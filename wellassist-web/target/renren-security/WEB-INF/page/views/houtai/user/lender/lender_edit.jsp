		...
		<div class="box box-solid">
        	<div class="box-header with-border">
            	<i class="fa fa-text-width"></i>
             	<h3 class="box-title">放款家信息<c:if test="${userId==0}"></c:if>添加<c:if test="${userId!=0}">编辑</c:if></h3>
           	</div>
           	<div class="box-body">
           		<form id="lenderEditForm" name="lenderEditForm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/houtai/user/HoutaiUserLenderCtrl-setLenderInfo">
...
<script type="text/javascript">
 		...
 		$("#lenderEditForm").validate({
		    ...
		    submitHandler: function(form){
		    	...
		    	$.post($(form).attr("action"),$(form).serialize(),function(data){
		    		...
		    		alert(data.content);
		            if(data.status=="1"){
		            	window.location.href = "${pageContext.request.contextPath}/houtai/user/HoutaiUserLenderCtrl-getLenderList";
		            }
		      	})
		      	.error(function(data){
		      		alert("操作失败！")
 	...
 	function uploadImage(idx){
 		...
 		$("#lenderEditForm").ajaxSubmit(options);
