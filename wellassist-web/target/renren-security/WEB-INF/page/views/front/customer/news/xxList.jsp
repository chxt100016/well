...
<div id = "content-rect">
	...
	<form id="searchFrm" method="post" action="${pageContext.request.contextPath}/front/customer/NewsController-xxList">
	...
	$(function(){
 		...
 		$("#selReadBtn").click(function(){
 			...
 			if(ids!=""){
 				if(confirm("你要确定已读操作吗?")){
 					$.post("${pageContext.request.contextPath}/front/customer/NewsController-setIsRead",{ids:ids},function(data){
 			    		data = $.parseJSON(data);
 			            if(data.status=="1"){
			...
			var ids = "";
 			...
 			if(ids!=""){
 				if(confirm("你要确定删除操作吗?")){
 					$.post("${pageContext.request.contextPath}/front/customer/NewsController-delXx",{ids:ids},function(data){
 			    		data = $.parseJSON(data);
 			            if(data.status=="1"){
 			            	searchData(1);
 						}
		...
		$("#allDelBtn").click(function(){
			if(confirm("你要确定所有删除操作吗?")){
				$.post("${pageContext.request.contextPath}/front/customer/NewsController-delXx",{ids:0},function(data){
		    		data = $.parseJSON(data);
		            if(data.status=="1"){
		            	searchData(1);
					}
	...
	function viewContent(txId){
		if($("#newsincontent_" + txId).css('display')=='none'){
			if($("#isReadHidden_" + txId).val()=="0"){
				$.post("${pageContext.request.contextPath}/front/customer/NewsController-setIsRead",{ids:txId},function(data){
		    		data = $.parseJSON(data);
		            if(data.status=="1"){
