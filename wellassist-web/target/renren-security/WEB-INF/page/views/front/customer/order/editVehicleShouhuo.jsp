					...
					<div style="border:solid 1px #d0d0d0;font-size:24px;margin:16px 48px;overflow:auto;">
						<div style = "border-bottom: solid 1px#E0E0E0; overflow:auto;">
							...
							<div class="grayboxwithoutleft" style="width:15%;line-height:80px;font-size:14px;float:left;border:none;text-align:center;">
								<a style="cursor:pointer;color:black;" onclick="toURL('qrsh', '${item.zorderId}', '${item.vehicleId}')">确认</a>
							</div>
						</div>
					</div>
				</c:forEach>
...
<script type="text/javascript">
	...
	function toURL(action, zorderId, vehicleId){
		...
		if(zorderId!=''){
			if(action=='qrsh'){
				if(confirm("你要确定操作吗?")){
					$.post("${pageContext.request.contextPath}/front/customer/CustomerBackOrderCtrl-setVehicleShouhuo",{vehicleTrans:vehicleTrans,orderId:orderId,zorderId:zorderId, vehicleId:vehicleId},function(data){
			    		data = $.parseJSON(data);
			    		alert(data.content);
			            if(data.status=="1"){
