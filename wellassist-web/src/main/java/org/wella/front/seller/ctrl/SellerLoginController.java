...
import org.wella.front.seller.mapper.SellerLoginMapper;
...
public class SellerLoginController extends BaseController {
    ...
    @RequestMapping(
            value = {"/front/seller/SellerLoginController-loginNext"},
        ...
        if(!username.equals("") && !password.equals("")) {
            try {
                ...
                if(!CommonUtil.check_inj_request_param(e)) {
                    String strsql = "SELECT * FROM wa_user inner join wa_userinfo on wa_user.user_id = wa_userinfo.user_id WHERE user_type = 0 AND (user_phone = \'" + username + "\' OR user_email = \'" + username + "\')";
                    e.put("strsql", strsql);
