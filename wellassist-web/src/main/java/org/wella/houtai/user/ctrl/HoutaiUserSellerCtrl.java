...
@Controller
public class HoutaiUserSellerCtrl extends BaseController {
    @Autowired
    private UploadController uploadController;
    @Autowired
    private HoutaiUserSellerMapper sellerMapper;

    public HoutaiUserSellerCtrl() {
    }

    @RequestMapping({"/houtai/user/HoutaiUserSellerCtrl-getSellerList"})
    public String getSellerList(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        return "views/houtai/user/seller/seller_list.jsp";
    }

    @RequestMapping({"/houtai/user/HoutaiUserSellerCtrl-getSellerDetail"})
    public String getSellerDetail(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        return "views/houtai/user/seller/seller_detail.jsp";
    }

    @RequestMapping(
            value = {"/houtai/user/HoutaiUserSellerCtrl-setUserState"},
            method = {RequestMethod.POST}
    )
    public void setUserState(HttpServletRequest request, HttpServletResponse response, Model model) {
        String ret = "-1";
        JSONObject obj = new JSONObject();
        obj.put("content", ConstantUtil.MSG_PARAM_ERR);
        String userId = CommonUtil.GetRequestParam(request, "userId", "0");
        String userState = CommonUtil.GetRequestParam(request, "userState", "0");
        if(CommonUtil.getIntFromString(userId) > 0) {
            try {
                HashMap e = new HashMap();
                HashMap mapClass = new HashMap();
                mapClass.put("user_state", userState);
                e.put("mapClass", mapClass);
                e.put("tableName", "wa_user");
                e.put("keyName", "user_id");
                e.put("keyValue", userId);
    ...
    @RequestMapping({"/houtai/user/HoutaiUserSellerCtrl-editSellerInfo"})
    public String editSellerInfo(HttpServletRequest request, HttpServletResponse response, Model model) {
        String userId = CommonUtil.GetRequestParam(request, "userId", "0");
        Map param;
        if(CommonUtil.getIntFromString(userId) > 0) {
        ...
        return "views/houtai/user/seller/seller_edit.jsp";
    }

    @RequestMapping(
            value = {"/houtai/user/HoutaiUserSellerCtrl-setSellerInfo"},
            method = {RequestMethod.POST}
    )
    public void setSellerInfo(HttpServletRequest request, HttpServletResponse response, Model model) {
        String ret = "-1";
        JSONObject obj = new JSONObject();
        obj.put("content", ConstantUtil.MSG_PARAM_ERR);
        String userId = CommonUtil.GetRequestParam(request, "userId", "0");
        ...
        try {
            ...
            if(userId.equals("0")) {
                ...
                if(ret.equals("-1")) {
                    ...
                    mapClass.put("user_type", Integer.valueOf(0));
    ...
    @RequestMapping(
            value = {"/houtai/user/HoutaiUserSellerCtrl-getRegionList"},
            method = {RequestMethod.POST}
    )
    public void getRegionList(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        ArrayList regionList = this.sellerMapper.getRegionList(param);
