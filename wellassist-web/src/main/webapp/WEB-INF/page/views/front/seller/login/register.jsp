...
.companyImg{
	...
	<div class="main-wrapper" style="margin-left: 0px;">
		...
		<div class="abovenavbar">
			<span style="margin-left: 5%;">WellAssist供应链系统(卖家端)</span>
			...
			<form method="post" action="javascript:myfunc();" id="sender_register" method="post">
				...
				<div id="kehuzhuce_page1">
					...
					<div align=left style="margin-top: 70px; margin-bottom: 48px; width: 800px; height: 80%; background: white; box-shadow: 0 -1px 6px rgba(0, 0, 0, 0.5);">
						...
						<table style="margin-left: 15%; margin-right: 15%; width: 100%;position:relative;">
							<tr>
								...
								<td style="width: 85%;"><input type="text"
									name="companyname" style="width: 50%;" placeholder="请输入企业名称"></td>
							</tr>
							<tr>
						...
						<div class="box-in-level1 titlewithline" style="margin-left: 7%; margin-top: 40px; margin-bottom: 16px;background: url('${pageContext.request.contextPath}/resources/wella/front/images/breakline.png') no-repeat 93px 50%;background-size:600px 10px;background-position-y:6px;">密码信息</div>
					    	...
					    	var contactemail =$("input[name='contactemail']").val();
				 			...
				 			$.post("${pageContext.request.contextPath}/front/sender/SenderLoginController-onCheckMobileEmail", {contactphone:contactphone,contactemail:contactemail},	function(data) {
								if(data.state == -3 || data.state == -4) {
									...
									$(form).attr("action", "<c:url value="/front/seller/SellerLoginController-registerNew"/>");
									$.post($(form).attr("action"),$(form).serialize(),function(data){
							    		data = $.parseJSON(data);
							    		if(data.state==1){
							            	window.location.href = "${pageContext.request.contextPath}/front/seller/SellerLoginController-registerNext";
							            }else{
							            	alert(data.content);
							            }
							      	})
