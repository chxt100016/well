...
<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
	...
	<!-- 			<th class = "center" style = "width:70px;">ä»»å¡</th> -->
				...
				<c:if test = "${searchType == 4}"><th class = "center" style = "width:60px;">激活</th></c:if>
		...
		<c:forEach var="item" items="${arrayList}" varStatus="status">
			<tr>
				...
				<c:if test="${item.adminName==Null}"><td class = "center">无</td></c:if>
				<c:if test="${item.adminName!=Null}"><td class = "center">${item.adminName}</td></c:if>
				<c:if test="${item.ghfName==Null}"><td class = "center">无</td></c:if>
				<c:if test="${item.ghfName!=Null}"><td class = "center">${item.ghfName}</td></c:if>
				<td class = "center">
					<c:if test="${item.state==0}"><span class="label label-warning">未审核</span></c:if>
					<c:if test="${item.state==1}"><span class="label label-info">审核中</span></c:if>
					<c:if test="${item.state==2}"><span class="label label-info">审核中</span></c:if>
					<c:if test="${item.state==3}"><span class="label label-success">已审核</span></c:if>
					<c:if test="${item.state==4}"><span class="label label-error">不通过</span></c:if>
				</td>
				<c:if test = "${searchType == 4}">
				<td class = "center">
	   				<c:if test="${item.isLock==0}"><span class="label label-important">可用</span></c:if>						
					<c:if test="${item.isLock==1}"><span class="label label-inverse">不可用</span></c:if>													
				</td>
				</c:if>
				<td class = "center">
					<a href="javascript:onGetDetailData(${item.id})" class="btn mini icn-only"  title="编辑"><i class="icon-share"></i></a>
					<c:if test="${searchType!=3&&searchType!=4}"><a href="javascript:onSetStatus(${item.id},${item.state})" class="btn mini purple icn-only" title="审核"><i class="icon-filter icon-white"></i></a></c:if>
					<c:if test = "${searchType == 4}">
	...
	<!-- 			<th class = "center" style = "width:70px;">ä»»å¡</th> -->
				<th style = "" class="sortButton" value="regTime">注册时间</th>				
				<th class = "center" style = "width:60px;">激活</th>
				<th class = "center" style = "width:200px;">操作</th>
			</tr>
		</thead>
		<tbody>
