...
Point.prototype = {
	...
	init: function (series, options, x) {
		...
		/**************************  dondolari start  *************************/
		if(series.options.type == 'column' && options.length == 3) {
			if(options[2] == 0) point.color = '#0ab92b';
			else point.color = '#f01717';
		}
		/**************************  dondolari end  *************************/
	...
	generatePoints: function () {
		...
		for (i = 0; i < processedDataLength; i++) {
			...
			if (!hasGroupedData) {
				...
				/**************************  dondolari start  *************************/
                if(series.options.type == 'column' && dataOptions[cursor].length == 3) {
                	points[i] = (new pointClass()).init(series, [processedXData[i]].concat(splat(processedYData[i])).concat(dataOptions[cursor][2]));
                } 
                /**************************  dondolari end  *************************/	
			}
		}
