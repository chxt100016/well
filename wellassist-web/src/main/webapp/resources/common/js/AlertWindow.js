...
function ShowWindowAlert_new(title, msgcontent, btn_cancel,callback) {
	...
	content += ' 			    <div class="modal modal-dialog clearfix" id="" style="top:20%!important;">';
	content += ' 				  <div class="modal-dialog-message" style="width:120%!important;">';
	content += ' 					<div align=center style="width:100%; background: white;box-shadow: 2px 2px 8px #808080;padding-bottom:16px;">';
	content += ' 					<div><img src="/wella/resources/wella/front/images/zhuceok_icon.png" style="margin-top: 45px;margin-bottom: 45px;"></div>';
	content += ' 					<div style="font-size:32px;margin-bottom: 50px;">'+msgcontent+'</div>';
	content += ' 					<div id="backtodingdanlist" class="submit" style="letter-spacing: 10px;font-weight:bold;font-size:25px;margin-bottom: 35px;color:#0077dd;cursor:pointer;">'+btn_cancel+'</div>';
	content += ' 				  </div>';
	content += ' 			  </div>';
	content += ' 			</div>';
